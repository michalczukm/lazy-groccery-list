...
// worker-configuration.d.ts (generated by `wrangler types` from wrangler.jsonc) doesn't
// declare every var wrangler.test.jsonc provides — cast to this module's own Env instead
// of widening the generated global type just for this test.
...
describe('POST /api/categorize', () => {
  ...
  describe('upstream failure', () => {
    ...
    it('fires worker_request_error to PostHog when Mistral upstream fails', async () => {
      ...
      const cookie = await signSession(testEnv.SESSION_HMAC_SECRET, Math.floor(Date.now() / 1000))
